...
class PauseView(arcade.View):
    ...
    def __build_pause_screen(self):
        ...
        views_button = arcade.gui.UIAnchorWidget(child=views_button, anchor_x="center", anchor_y="center", align_y=-110)

        quit_button = arcade.gui.UIFlatButton(text="Quit", width=200, height=50)
        quit_button.on_click = lambda _: arcade.close_window()
        quit_button = arcade.gui.UIAnchorWidget(child=quit_button, anchor_x="center", anchor_y="center", align_y=-165)
    ...
    def __build_view_select_menu(self):
        menu = arcade.gui.UIBoxLayout()
        menu.add(arcade.gui.UITextArea(text="Views", font_size=20, font_name="Arial", color=arcade.color.WHITE))

        # settings menu close button
        close_button = arcade.gui.UIFlatButton(text="Close", width=200, height=50)
        close_button.on_click = lambda _: self.uimanager.remove(menu)

        menu.add(arcade.gui.UIAnchorWidget(child=close_button, anchor_x="center", anchor_y="right", align_x=self.window.width/1.6))
        ...
        for i, view_name in enumerate(self.window.views):
            ...
            menu.add(arcade.gui.UIAnchorWidget(child=view_button, anchor_x="center", anchor_y="right", 
                                               align_x=self.window.width/1.6, align_y=self.window.height-(i+1)*15))
    ...
    def __resume_game(self, event):
        ...
        self.window.show_view(self._selected_view or self.window._last_view or "primary")
