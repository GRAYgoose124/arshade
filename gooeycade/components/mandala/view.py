...
class MandalaView(ShaderViewComponent):
    ...
    def setup(self):
        ...
        self.program = self.window.ctx.load_program(
            vertex_shader=default_root / descr.vert_shader,
    ...
    def __build_mesh(self, description: ProgramDefinition):
        ...
        vao = self.window.ctx.geometry(
            [arcade.gl.BufferDescription(buffer, *description.description)]
        )
    ...
    def on_update(self, delta_time: float):
        ...
        if self.__modelview_enabled:
            theta = (
                sin(
                    self.program["time"]
                    * self.description.rotation_speed
                    * (1 / self.description.speed)
                )
                * 2
                * pi
            )
            model = Mat4.from_rotation(
                theta * cos(self.program["time"]), Vec3(-0.1, 1.0, 0.5)
            ).translate(
                Vec3(0, 0.2 * theta, 0)
            )  # .scale(Vec3(2, 2, 2))
