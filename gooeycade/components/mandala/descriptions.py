...
class ProgramDefinition:
    ...
    speed: float = 1.0
    render_modes: tuple[int, int] = field(
        default=(pyglet.gl.GL_LINE_STRIP, pyglet.gl.GL_POINTS), kw_only=True
    )
...
class ParallelSpiralOrbit(ProgramDefinition):
    ...
    description: tuple[str, list[str]] | None = field(
        default=("f 3f 3f", ["in_id", "in_pos", "in_col"])
    )
...
class SinCosOrbit(ProgramDefinition):
    ...
    description: tuple[str, list[str]] | None = field(
        default=("f 3f 3f", ["in_id", "in_pos", "in_col"]), kw_only=True
    )
    render_modes: tuple[int, int] = field(
        default=(pyglet.gl.GL_POINTS, pyglet.gl.GL_TRIANGLE_STRIP), kw_only=True
    )
    modelview_enabled: bool = False
    point_size: float = 4.0
    speed: float = 1.0
